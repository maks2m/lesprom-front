    ...
    <table class="table table-striped table-hover table-sm">
      ...
      <tr>
        <th v-for="item in arrSeparateAbc"
          ...
          <div v-show="item.showIcon">
            <span v-if="item.separated === 'asc'">\/</span>
            <span v-else-if="item.separated === 'desc'">/\</span>
          </div>
        </th>
      </tr>
      </thead>
      ...
      <tr v-for="(item) in items" :key="item.id" v-if="items.length"
        ...
        <td>
          ...
          <router-link class="btn btn-info m-1" :to="{ name: 'order-edit-add-employees', params: {id: item.id} }">
